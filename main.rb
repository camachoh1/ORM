...
Person.create("persons", [ 
  {col_name: "name", value: "tu mae"},
  {col_name: "age", value: 80}
])
...
# find first n persons (will return the first person if no limit provided)
p "first two"
first_person = Person.first("persons", 2) 
p first_person


# find person by property returns the first record that meets the condition.
coleto = Person.find_by("persons", "name", "coleto")
p "by name"
p "coleto: #{coleto}"

# find person by property returns all users that meet the criteria sorted by the provided entry

filtered_person = Person.where("persons",[
  {col_name: "name", value: "pablo"},
  {col_name: "age", value: 40}], 
  {by: "age", order: "desc"})
p "where"
p filtered_person

Person.update("persons", [
  {col_name: "name", value: "anacleto"},
], [{col_name: "id", value: "3"}])

p "after update"
p Person.all("persons")

Person.delete("persons", [{col_name: "id", value: 2}])
